...
Genome::Genome(Genome* mere, Genome* pere) {
    ...
    if(pere != nullptr and mere != nullptr) {
        ...
        for(size_t i(0); i < TAILLE_PROFIL_IMMU; ++i) {
            ...
            profilImmunitaire[i] += uniform(-getAppConfig().genome_mutability_innate_immune_system,
                                              getAppConfig().genome_mutability_innate_immune_system);
        }
    } else {
        randomSexe();
        randomColor(0);
...
void Genome::setProfilImmunitaire(std::array<double, TAILLE_PROFIL_IMMU> const& profil) {
  for (size_t i(0); i < TAILLE_PROFIL_IMMU; ++i) {
    profilImmunitaire[i] = profil[i];
  }
}
...
void Genome::afficheGenome(std::ostream& sortie) const {
    ...
    afficheImmuneProfile(sortie);
}

void Genome::afficheImmuneProfile(std::ostream& out) const {
  out << "Immune profile = ";
  for(size_t i(0); i < TAILLE_PROFIL_IMMU; ++i) {
