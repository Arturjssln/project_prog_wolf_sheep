...
//CONSTRUCTEUR
Animal::Animal(Vec2d position, double energyLevel, double magnitude, Genome* pere, Genome* mere, int numTroupeau, double quenchLevel, Etat state)
: LivingEntity(position, energyLevel), direction(Vec2d(1,0)), genome(Genome(pere, mere)), state(state),
  ...
  immunity(this), quenchLevel(quenchLevel), magnitude(magnitude),
  currentTarget(Vec2d(1,0)), positionCible(Vec2d(0,0)), typeInSight(nourriture),
  closestPartner(nullptr), closestFood(nullptr), closestDrink(nullptr)
{
...
double Animal::getMaxSpeed() const {
	if (energyLevel < getAnimalCriticalEnergyThreshold() or quenchLevel < getAnimalCriticalQuenchThreshold())
	...
	switch (state) {
      ...
      break;
    case DRINK_IN_SIGHT : return getStandardMaxSpeed() * 3;
  		break;
...
double Animal::getAnimalCriticalQuenchThreshold() const {
  return getAppConfig().animal_min_quench * 2;
}
...
std::array<double,7> Animal::getAnimalData() const {
  std::array<double,7> data = {{ energyLevel,
                                quenchLevel,
...
bool Animal::isAlive() const {
  if(energyLevel < getAppConfig().animal_min_energy or
     quenchLevel < getAppConfig().animal_min_quench or
...
//Retourne si l'animal a soif ou non
bool Animal::quenched() const {
  //si l'animal est en train boire, il boit jusqu'à atteindre le seuil max
  if ((state == DRINKING) and (quenchLevel < getAppConfig().animal_quench_max)) return false;
  if (quenchLevel < getAppConfig().animal_quench_min) return false;
  return true;
}
...
/**************************************************************************************************************/
//METHODES DE MISE À JOUR (UPDATE)
...
void Animal::update(sf::Time dt) {
    ...
    Vec2d f;
    switch (state) {
    //Dans les cas où le leader est en FEEDING, DRINKING, MATING et GIVING_BIRTH, il sera à l'arrêt, les autres ne doivent donc plus le suivre
    //sinon on aurait un amas d'animaux d'où la mise à jour de l'attribut isAbleToBeFollowed
    //Pareil pour le cas HALLUCINATING -> aucune raison de le suivre étant donné qu'il ne suit pas de direction "normale"
      case FOOD_IN_SIGHT :
      ...
      case WANDERING :
        ...
        if(!isFree() and (getAppEnv().getLeader(this->getNumHerd()))->getIsAbleToBeFollowed()) {
          //On rajoute la force liée au leader
      ...
      case HALLUCINATING :
        if(isLeader()) setIsAbleToBeFollowed(false);
        decreasePoisonedTime(dt);
        breakTimeAfterDoingSomething -= dt;
        positionCible = randomPoisonedWalk();
        f = calculForce();
        break;
...
void Animal::updateDonnees(sf::Time dt, const Vec2d& force) {
  ...
  quenchLevel -= getAppConfig().animal_base_water_consumption + magnitude * getWaterLossFactor() * dt.asSeconds();
...
void Animal::updateState() {
  ...
  //Si il a fait quelque chose qui nécessite un temps de pause après, il ne fait rien tant que ce temps n'est pas écoulé
  } else if (breakTimeAfterDoingSomething > sf::Time::Zero or breakTime > sf::Time::Zero) {
    //On ne change pas l'état : l'animal ne fait rien pendant sa pause

  } else {
    //Avant de repasser en WANDERING si l'animal était dans un des 4 états dans les conditions ci-dessous,
    //l'animal pourra passer en IDLE avec une certaine probabilité (s'il n'a ni faim ni soif)
    if (((state == FEEDING) or (state == GIVING_BIRTH) or (state == MATING) or (state == DRINKING)) and (satisfied() and quenched())) {
    ...
    if(breakTime <= sf::Time::Zero) {
      if (isPregnant() and (gestationTime <= sf::Time::Zero)) {
      ...
      } else if (closestEnemies.empty() and closestPartner == nullptr and closestFood == nullptr and closestDrink == nullptr) {
        ...
        switch (typeInSight) {
          ...
          //Si il y a à boire dans son champ de vision mais pas d'ennemis, ni de partenaire, ni de nourriture
          case drink :
            if(quenched()) {
              state = WANDERING;
            } else if (isColliding(*closestDrink)) {
              state = DRINKING;
              isDrinking(closestDrink);
            } else {
              state = DRINK_IN_SIGHT;
            }
            positionCible = closestDrink->getPosition();
            break;
...
void Animal::updateClosest(std::list<LivingEntity*> ensembleEntities) {
  ...
  Vec2d posPlusProcheDrink = ConvertToGlobalCoord(posPlusProche);

  closestEnemies.clear();
  closestPartner = nullptr;
  closestFood = nullptr;
  closestDrink = nullptr;
  neighbors.clear();
      ...
      for (auto entite : ensembleEntities) {
        if (isTargetInSight(entite->getPosition())) {
          ...
          //PARTNER
          //Les animaux ne se reproduisent qu'au printemps
          if (getAppEnv().getSeason() == SPRING and
             (matable(entite)) and
          ...
          //DRINK
          if ((drinkable(entite)) and
            ((position - entite->getPosition()).lengthSquared() < (position - posPlusProcheDrink).lengthSquared()))
          {
              posPlusProcheDrink = entite->getPosition(); //on mémorise la position de l'entité la plus proche
              closestDrink = entite;
  	      }
...
Vec2d Animal::calculForce() const {
	...
	return (vTarget - getSpeedVector()) + calculAvoidanceForce();
}
...
//On définit une cible virtuelle qui permet de "marcher" sans réelle direction
Vec2d Animal::randomPoisonedWalk() {
  //Tous les vecteurs de cette méthode sont en coordonées locales
  Vec2d random_vec(uniform(-1.0,1.0), uniform(-1.0,1.0));
  currentTarget += random_vec * getRandomWalkJitter() * 50;
  currentTarget = currentTarget.normalised() * getRandomWalkRadius() * 5;
  Vec2d movedCurrentTarget = currentTarget;
  return ConvertToGlobalCoord(movedCurrentTarget); //Conversion en coordonnées globales
}
...
void Animal::analyzeEnvironnement() {
  ...
  } else if(closestDrink != nullptr) {
    typeInSight = drink;
  }
}

/**************************************************************************************************************/
//MÉTHODES DE DESSIN/AFFICHAGE
void Animal::drawOn(sf::RenderTarget& targetWindow) const {
  ...
  if (isDebugOn()) {
    ...
    if (state == HALLUCINATING) drawVirtualTarget(targetWindow);
...
void Animal::drawTextDebug(sf::RenderTarget& targetWindow) const {
  ...
  Vec2d positionTexteWater(-25 * direction + direction.normal() * getRadius() * 4);
  message = to_nice_string(quenchLevel);
  if (quenched()) {
    message += "    Quenched";
  } else {
    message += "    Thirsty";
  }
  auto textWater = buildText(message, ConvertToGlobalCoord(positionTexteWater),
                          getAppFont(), getAppConfig().default_debug_text_size, sf::Color::White);
  textWater.setRotation(getRotation() / DEG_TO_RAD + 90);


  targetWindow.draw(textEtat);
  targetWindow.draw(textSex);
  targetWindow.draw(textNrj);
  targetWindow.draw(textWater);
}
...
std::string Animal::displayState() const {
    switch (state) {
      case FOOD_IN_SIGHT : return "FOOD_IN_SIGHT";
        break;
      case DRINK_IN_SIGHT : return "DRINK_IN_SIGHT";
        break;
