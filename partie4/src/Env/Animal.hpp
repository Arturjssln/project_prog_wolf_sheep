...
class Animal : public LivingEntity {
	public:
		//CONSTRUCTEUR
		Animal(Vec2d position, int niveauEnergie, double magnitude, Genome* pere, Genome* mere, int numTroupeau, Etat etat = WANDERING);
    ...
    Vec2d calculPredatorForce() const;
		Vec2d calculForceEvitement() const;
		//DESSIN/AFFICHAGE
    ...
    void drawPregnantCircle(sf::RenderTarget& targetWindow) const;
		...
		std::vector<LivingEntity*> analyzeNeighborhood() const;
