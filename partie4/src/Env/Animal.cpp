...
//CONSTRUCTEUR
Animal::Animal(Vec2d position, int niveauEnergie, double magnitude, Genome* pere, Genome* mere, int numTroupeau, Etat etat)
: LivingEntity(position, niveauEnergie), genome(Genome(pere, mere)), etat(etat),
  breakTimeAfterDoingSomething(sf::Time::Zero), breakTime(sf::Time::Zero), gestationTime(sf::Time::Zero), nombreEnfants(0), pere(nullptr),
  pregnant(false), direction(Vec2d(1,0)), magnitude(magnitude), currentTarget(Vec2d(1,0)),
  numHerd(numTroupeau), leader(false), isAbleToBeFollowed(false)
{
...
void Animal::update(sf::Time dt) {
    ...
    //sinon on aurait un amas d'animaux d'où l'attribut isAbleToBeFollowed
      case FOOD_IN_SIGHT :
      ...
      case WANDERING :
        ...
        if(!isFree() and (getAppEnv().getLeader(this->getNumHerd()))->getIsAbleToBeFollowed()) {
          ...
          f += calculForce() + calculForceEvitement() ;
        }
        break;
...
void Animal::updateState() {
  ...
  if (breakTimeAfterDoingSomething > sf::Time::Zero or breakTime > sf::Time::Zero) {
    ...
    if (((etat == FEEDING) or (etat == GIVING_BIRTH) or (etat == MATING)) and (satisfied())) {
...
Vec2d Animal::calculForceEvitement() const {
  Vec2d force(0,0);
  std::vector<LivingEntity*> neighborhood(analyzeNeighborhood());
  for (size_t i(0); i < neighborhood.size(); ++i) {
    Vec2d toTarget(position - neighborhood[i]->getPosition());
    force += (COEFF_EVITEMENT * toTarget) / toTarget.lengthSquared();
  }
  return force;
}
...
std::vector<LivingEntity*> Animal::analyzeEnvironnement() {
  ...
  if(!plusProches.empty()) {
    ...
    return plusProches; //retourne le tableau d'ennmis dans le champ de vision
  }
...
std::vector<LivingEntity*> Animal::analyzeNeighborhood() const {
  std::vector<LivingEntity*> lesEntitesVisibles(getAppEnv().getEntitiesInSightForAnimal(this));
  return lesPlusProches(promenade, lesEntitesVisibles);
}
...
std::vector<LivingEntity*> Animal::lesPlusProches(TypeInSight elementRecherche, std::vector<LivingEntity*> ensembleEntities) const {
  ...
  switch (elementRecherche) {
    ...
    case promenade :
      for (auto entite : ensembleEntities) {
        if ((position - entite->getPosition()).lengthSquared() < pow(SEUIL_PROXIMITE ,2) and
            (position != entite->getPosition()))
        {
          plusProches.push_back(entite);
        }
      }
    break;
